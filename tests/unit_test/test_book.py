...
from tests.conftest import client, club, competition, competition_with_availability
...
def test_book_competition_without_availability(client, club, competition, monkeypatch):
    """
...
def test_book_valid_competition(client, club, competition_with_availability, monkeypatch):
    """
    Given: A user access the booking feature for a competition
    When: a competition is in the future
    Then: he should be redirected to the correct booking template
    """
    monkeypatch.setattr('server.competitions', competition_with_availability)
    monkeypatch.setattr('server.clubs', club)
    response = client.get("book/Future%20comp/Simply%20Lift")
...
def test_book_invalid_competition(client, club, competition_with_availability, monkeypatch):
    """
    Given: A user access the booking feature for a competition
    When: a competition is in the future
    Then: he should be redirected to the correct booking template 
